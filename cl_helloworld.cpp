...
{	
	...
	ret = LoadKernelSource ((char *)"/sdcard/hello_world.cl", &kernel_str);
	...
	if (ret != CL_SUCCESS) 
	{
		 printf ("Failed Creating hellow_world program : %d\n", ret);
		 exit (0);
	}
	else
		printf (" Ok\n");
...
cl_int BuildProgram (cl_program *program, cl_device_id *device_id, cl_context context, kernel_src_str *kernel)
{
	...
	if (error != CL_SUCCESS)
	{
		printf ("clCreateProgramWithSource error: %d", error);
		return CL_ERROR;
	}
	...
	if (error < 0)
	{
		...
		//---------------------------------------------------
         ...
         printf("clGetProgramBuildInfo error: %d\n", ret);
         printf("len:%d\n", len);
         printf("%s\n", buffer);
	}
